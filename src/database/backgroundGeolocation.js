...
async function saveDevice(device_update) {
  try {
    const db = await Database.get()
    var doc = await db.device_updates.insert(device_update);
    console.log(doc);
    device.lat = device_update.lat
    device.lon = device_update.lon
    var device = await db.device.updates.insert(device)
  } catch (err) {
    console.log(err);
  }
}
